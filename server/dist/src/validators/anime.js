...
const AnimeSchema = z.object({
    ...
    statusName: z.enum(['Cancelled', 'Ongoing', 'Completed', 'Upcoming'], { message: "Status must be one of: Cancelled, Ongoing, Completed, or Upcoming." }),
    ...
    releaseDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Release date must be in the format YYYY-MM-DD." }),
    episodeCount: z.string().regex(/^\d+$/, { message: "Episode count must be a positive integer." }),
    durationPerEpisode: z.string().regex(/^\d+$/, { message: "Duration per episode must be a positive integer in minutes." }),
